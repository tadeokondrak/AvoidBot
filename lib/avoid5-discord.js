...
exports.handleMessage = message => {
  ...
  if (results.count >= 8 || results.longestRun >= 3 || results.bolded) {
    let response = 'Too many fifthglyphs, or a bold glyph. Auto-kick.';
    message.channel.sendMessage(response);
    message.author.send(response);
    message.client.channels.get(config.discord.logChannel).sendMessage(message.author + ' was kicked.');
    message.member.kick();
...
const stripMentions = function (text) {
  let regex = /\s*(?:@(?:here|everyone)|<(?:@[!&]?|#|:.*?:)\d+>)\s*/ig;
